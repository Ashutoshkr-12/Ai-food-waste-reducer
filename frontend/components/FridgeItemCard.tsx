...
export default function FridgeItemCard({
  ...
  return (
    <div
      className={`relative bg-linear-to-br ${getBglinear(expiry_days)} rounded-[2rem] p-6 border-2 ${getBorderColor(expiry_days)} shadow-xl hover:shadow-2xl overflow-hidden`}>
      ...
      : expiry_days === 1
        ...
        : `${expiry_days} days left`}
          ...
          <div className="relative h-3 bg-[#FFFCF8]/60 backdrop-blur-sm rounded-full overflow-hidden shadow-inner border border-[#FFFCF8]">
            <div
              className={`h-full bg-linear-to-r ${getProgresslinear(expiry_days)} rounded-full shadow-lg`}
              style={{ width: `${getProgressWidth(expiry_days)}%` }}
            />
          </div>
          ...
          <div className="flex items-center justify-between">
            <span className="inline-flex items-center gap-2 text-sm font-bold px-4 py-2 rounded-xl bg-[#FFFCF8]/70 backdrop-blur-sm shadow-lg border border-[#FFFCF8]">
             {ingredient.confidence ? `${Math.round(ingredient.confidence! * 100)}% confident`: <></>}
            </span>
              ...
              {[...Array(3)].map((_, i) => (
                <div
                  ...
                  className={`w-2 h-2 rounded-full ${
                    i < Math.ceil(expiry_days / 2.5)
                      ? `bg-linear-to-r ${getIconlinear(expiry_days)} shadow-md`
                      : "bg-gray-300"
                  }`}
                />
              ))}
